...
## 在 Google Slides 中對圖案進行標注

製作模板時，需要在灰色矩形上標注「IMAGE」等提示文字，以下是操作方法。

### 方法一：直接在形狀上打字（最簡單）

Google Slides 中，**任何形狀都可以直接輸入文字**：

1. 插入一個矩形（**插入** → **形狀** → **矩形**）
2. **雙擊**該矩形 → 游標出現在形狀內部
3. 直接打字，例如輸入 `IMAGE` 或 `圖片區域`
...
- 選取文字 → 調整字體大小、顏色、粗細
- 上方工具列 → 對齊方式（置中、靠左等）
- 建議用淺灰色文字 + 粗體，讓它看起來像「提示標注」而不是正式內容
...
在主題編輯器中製作 `image_left` / `image_right` / `bullets` Layout 時，因為 Google Slides 可能不支援 Picture Placeholder，需要用灰色矩形代替。標注方式：

1. 插入矩形，設定填充色為淺灰 `#E2E8F0`
2. 雙擊矩形，輸入 `IMAGE`
3. 設定文字：
   - 字體大小：18pt
   - 顏色：深灰 `#94A3B8`
...
1. 右鍵矩形 → **格式選項 (Format options)**
2. 展開 **文字適應 (Text fitting)**
3. 將垂直對齊設為 **置中 (Middle)**

或者用上方工具列的垂直對齊按鈕（看起來像有上下箭頭的橫線圖示）。

### 重要提醒

在主題編輯器（母版模式）中加入的這些標注矩形是**靜態形狀**，它們會出現在每張使用該 Layout 的投影片上，但**程式碼無法透過 `placeholders[idx]` 存取**。它們只是視覺提示，告訴使用者「這裡預留給圖片」。未來如果要程式化插入真實圖片，需要用 python-pptx 腳本額外補上 Picture Placeholder，或改用 PowerPoint 製作模板。
